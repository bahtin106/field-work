...
export default function DispatcherHome({ fullName }) {
  ...
  if (blocked) {
    return (
      <View style={styles.blocked}>
        ...
        <Pressable onPress={logout} style={({pressed}) => [styles.appButton, styles.btnDanger, pressed && {transform:[{scale:0.98}]}]}>
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Здравствуйте, {fullName}</Text>
      <Text style={styles.text}>Вы можете управлять заявками и назначать исполнителей.</Text>

      <Pressable onPress={logout} style={({pressed}) => [styles.appButton, styles.btnDanger, pressed && {transform:[{scale:0.98}]}]}>
        <Text style={styles.appButtonText}>Выйти из профиля</Text>
      </Pressable>

      {/* Модалка на будущее, если понадобится показывать блокировку поверх экрана */}
