...
function joinName(obj){
  ...
  const parts = [
    obj.full_name, obj.fullName, obj.name,
...
function extractStatus(order, getFieldByKey, pills){
  ...
  const candidates = [pillCandidate, 'status_v2', 'status', 'state', 'order_status', 'orderState'].filter(Boolean)
...
export default function DynamicOrderCard({
  ...
  const preset = {
    fields: Array.isArray(presetRaw?.fields) && presetRaw.fields.length > 0
      ? presetRaw.fields.filter(k => k !== 'datetime')
      : ['title','customer_name','address'],
    pills: Array.isArray(presetRaw?.pills) && presetRaw.pills.length > 0
      ? presetRaw.pills
      : ['status'],
    secondary: Array.isArray(presetRaw?.secondary) && presetRaw.secondary.length > 0
      ? presetRaw.secondary
      : ['assigned_to_name'],
  }
  ...
  const primaryRows = useMemo(() => {
    return fields.map((key) => {
      ...
      const label = (field?.label && field.label !== key) ? field.label : (RUS_LABELS[key] || (field?.label ?? key))
      return { key, label, value }
    }).filter(r => r.key !== 'title')
  }, [fields, order, getFieldByKey])

  // Status pill
  const statusTitle = useMemo(() => extractStatus(order, getFieldByKey, pills), [order, pills, getFieldByKey])
  const statusBadge = useMemo(() => {
    if (!statusTitle) return null
    let bg = '#E8F0FE', text = '#0A84FF' // Новый — синий
    if (/лент/i.test(statusTitle)) { bg = '#FFF7CC'; text = '#8A6D1F' }     // В ленте — жёлтый
    else if (/заверш/i.test(statusTitle)) { bg = '#F2F2F7'; text = '#8E8E93' }   // Завершённая — серый
    else if (/работ/i.test(statusTitle)) { bg = '#E9F7EF'; text = '#34C759' } // В работе — зелёный
  ...
  const executorName = useMemo(() => {
    ...
    const emailLike = order?.assigned_to_email || order?.executor_email || order?.assignee_email || order?.assigned_to_login || order?.executor_login
  ...
  const roleRaw = (viewerRole || order?.viewerRole || order?.current_user_role || order?.role || settings?.role || settings?.currentRole || '').toString().toLowerCase();
  ...
  const initialExecCached = (order?.assigned_to && EXECUTOR_NAME_CACHE.get(order.assigned_to)) || ''
  ...
  return (
    ...
    >
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
        <Text numberOfLines={1} style={{ fontSize: 16, fontWeight: '600', color: theme.colors.text, flex: 1, paddingRight: 8 }}>
          {title || '—'}
        </Text>
        {statusBadge && (
          <View style={{ backgroundColor: statusBadge.bg, borderRadius: 999, paddingHorizontal: 10, paddingVertical: 6 }}>
            <Text style={{ fontSize: 12, fontWeight: '700', letterSpacing: 0.3, color: statusBadge.text }}>
              {statusBadge.title}
            </Text>
          </View>
        )}
      ...
      <View style={{ gap: 2, marginTop: 4, marginBottom: 6 }}>
        {primaryRows.map((row) => (
          <Text key={row.key} numberOfLines={1} style={{ fontSize: 14, color: theme.text.muted.color }}>
            {row.label ? `${row.label}: ` : ''}
      ...
      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: 6 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          {showUrgentDot && (
            <View style={{ width: 18, height: 18, borderRadius: 9, backgroundColor: '#FF3B30', alignItems: 'center', justifyContent: 'center', marginRight: 6 }}>
              <Text style={{ color: '#fff', fontSize: 12, fontWeight: '700', lineHeight: 12 }}>с</Text>
            </View>
          )}
          {showDate ? (
