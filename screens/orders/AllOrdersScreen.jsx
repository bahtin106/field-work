...
function AllOrdersContent() {
  ...
  const initialAllOrdersScopeKey = useMemo(() => {
  ...
  const hasInitialRouteFilters =
    effectiveStatusFilter !== 'all' ||
    Boolean(
      readRouteParam(filter) && normalizeAllOrdersStatusFilter(readRouteParam(filter)) !== 'all',
    ) ||
    readRouteListParam(statuses).some((value) => normalizeAllOrdersStatusFilter(value) !== 'all') ||
    Boolean(readRouteParam(department)) ||
    ...
    Boolean(readRouteParam(departure_time_from)) ||
    Boolean(readRouteParam(departure_time_to)) ||
    Boolean(readRouteParam(created_date_from)) ||
    Boolean(readRouteParam(created_date_to)) ||
    Boolean(readRouteParam(created_time_from)) ||
    Boolean(readRouteParam(created_time_to)) ||
    Boolean(readRouteParam(sum_min)) ||
    Boolean(readRouteParam(sum_max)) ||
    hasLinkedRelationFilter;
  const initialAllOrdersCache = useMemo(() => {
    if (hasInitialRouteFilters) return { found: false, rows: [] };
    const recent = queryClient.getQueryData(['orders', 'all', 'recent', initialAllOrdersScopeKey]);
    if (Array.isArray(recent)) return { found: true, rows: recent };
    return { found: false, rows: [] };
  }, [hasInitialRouteFilters, initialAllOrdersScopeKey, queryClient]);
  const [orders, setOrders] = useState(() => initialAllOrdersCache.rows);
  const [loading, setLoading] = useState(() => !initialAllOrdersCache.found);
  ...
  const feedScopeKey = initialAllOrdersScopeKey;
  ...
  const renderItem = useCallback(
    ...
    [companySettings, departureTimeEnabled, openOrderDetails, orderFieldsByKey, profile?.role, statusSystem.isEnabled, statusSystem.statuses],
  );
