...
import { sanitizeFieldLabel } from '../../src/features/fieldSettings/presentation';
...
function CreateOrderContent() {
  ...
  const isFieldRequired = useCallback(
    (key) => !AUTO_FILLED_ORDER_FIELDS.has(String(key || '')) && getField(key)?.required === true,
    [getField],
  );
  ...
  const getFieldLabel = useCallback(
    (fieldKey, fallback) => {
      ...
      const fieldLabel = sanitizeFieldLabel(sanitizeVisibleText(field?.label, ''));
      if (fieldLabel) return fieldLabel;

      return sanitizeFieldLabel(fallbackLabel);
    },
    [getField, t],
  );
  ...
  const validateRequiredFields = useCallback(() => {
    try {
      const requiredFields = (schema?.fields || []).filter(
        (f) => f?.required && !AUTO_FILLED_ORDER_FIELDS.has(String(f?.field_key || '')),
      );
  ...
  }, [schema, form, departureDate, departureTime, toFeed, assigneeId, normalizePhone, getFieldLabel, t, selectedClientId, selectedClientObjectId, withoutAddressSelected, draftClientObject, useWorkTypes, workTypeId, description, hasDepartureTimeValue]);
  ...
  useEffect(() => {
    ...
    (() => {
      ...
      const fields = (remoteFields.length ? remoteFields : DEFAULT_FIELDS)
        .filter((field) => !REMOVED_ORDER_ADDRESS_FIELDS.has(String(field?.field_key || '')))
        .map(normalizeCreateOrderField);
      if (!mounted) return;
