...
export default function MessengerBotSettingsScreen({ provider = 'telegram' }) {
  ...
  const providerKey = String(provider || '').toLowerCase() === 'max' ? 'max' : 'telegram';
  const translationPrefix =
    providerKey === 'max' ? 'company_settings_max' : 'company_settings_telegram';
  const tr = React.useCallback(
    (suffix, fallback) => t(`${translationPrefix}_${suffix}`, fallback),
    [t, translationPrefix],
  );
  const botIntegration = React.useCallback(
    (action, payload) => messengerBotIntegration(providerKey, action, payload),
    [providerKey],
  );
  ...
  React.useLayoutEffect(() => {
    nav?.setParams?.({ headerTitle: tr('title') });
  }, [nav, tr]);

  const friendlyLoadError = React.useCallback((error) => {
    const message = String(error?.message || '').trim();
    if (!message) return tr('load_failed');
    return message;
  }, [tr]);
  ...
  const load = React.useCallback(async ({ silent = false } = {}) => {
    ...
    setScreenError('');
    try {
      const next = await botIntegration('status');
  ...
  }, [botIntegration, friendlyLoadError, toast]);
    ...
    (async () => {
      try {
        ...
        for (let attempt = 0; attempt < 3; attempt += 1) {
          try {
            next = await botIntegration('status');
            break;
          } catch (error) {
            lastError = error;
  ...
  }, [botIntegration, friendlyLoadError]);
  ...
  const saveSnapshot = React.useCallback(async (
    ...
    setSaveState('saving');
    try {
      const next = await botIntegration('save_config', buildSavePayload(snapshot));
      confirmedDataRef.current = next;
  ...
  }, [botIntegration, buildSavePayload, markSaved, serializePayload, t, toast]);

  const toggleBotEnabled = React.useCallback(async (value) => {
    if (value === true && enabledAvailableFieldCount <= 0) {
      toast.info(tr('at_least_one_field_toast'));
      return;
    }
    const previous = data;
    const nextSnapshot = {
  ...
  }, [data, enabledAvailableFieldCount, saveSnapshot, setLocalData, toast, tr]);
  ...
  const regenerateLink = React.useCallback(async () => {
    ...
    try {
      const next = await botIntegration('regenerate_token');
      setData((prev) => ({
        ...(prev || {}),
  ...
  const handleFieldToggle = React.useCallback((field, value) => {
    ...
    if (field?.disabled_by_field_settings === true) {
      toast.info(tr('field_disabled_by_settings'));
      return;
    }
  ...
  }, [enabledAvailableFieldCount, toast, tr, updateField]);

  const saveStatusText = React.useMemo(() => {
    if (saveState === 'error') return tr('status_error');
    return '';
  }, [saveState, tr]);
  ...
  }, [base.label, base.row, handleFieldToggle, s, toast, tr]);
    ...
    const timeoutId = setTimeout(async () => {
      ...
      try {
        ...
        if (lastFailedPayloadRef.current !== currentPayload) {
          ...
          toast.error(tr('save_reverted'));
        }
      }
    }, 450);

    return () => clearTimeout(timeoutId);
  }, [data, loading, saveSnapshot, saving, serializePayload, toast, tr]);
  ...
  return (
    <Screen background="background" scroll={false}>
      <ScrollView contentContainerStyle={s.content} stickyHeaderIndices={stickyHeaderIndices}>
        {screenError ? (
          <Card style={s.errorCard}>
            <Text style={s.errorTitle}>{tr('load_failed')}</Text>
        ...
        <Card paddedXOnly style={s.heroCard}>
          <View style={base.row}>
            <Text style={base.label}>{tr('enabled')}</Text>
            <ThemedSwitch
            ...
            <>
              ...
              <View style={s.linkHeaderRow}>
                <View style={s.linkHeaderInline}>
                  <Text style={base.label}>{tr('link_label')}</Text>
                  <IconButton
                    onPress={confirmRegenerateLink}
                    accessibilityLabel={tr('regenerate_link')}
              ...
              <View style={s.linkValueRow}>
                ...
                >
                  <Text
                    ...
                    numberOfLines={3}
                  >
                    {data?.start_link || tr('link_unavailable')}
                  </Text>
                </Pressable>
                {data?.start_link ? (
                  <IconButton
                    style={{ display: 'none' }}
                    onPress={copyStartLink}
                    accessibilityLabel={tr('copy_link')}
                  >
        ...
        <View style={s.stickyHeaderShell}>
          <Card paddedXOnly style={s.fieldHeaderCard}>
            <View style={[base.row, s.fieldHeaderRow]}>
              ...
              <View style={s.fieldControls}>
                <Text style={s.fieldColumnTitle}>{tr('show_column')}</Text>
              </View>
            </View>
            <View style={base.sep} />
          </Card>
      ...
      <ConfirmModal
        ...
        title={tr('regenerate_confirm_title')}
        message={tr('regenerate_confirm_message')}
        confirmLabel={tr('regenerate_confirm_action')}
