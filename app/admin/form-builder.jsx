...
if (!supabase) {
  throw new Error("Supabase client import failed: check lib/supabase export (default or named) and import path.");
}
...
export default function FormBuilderScreen() {
  ...
  async function onDeleteConfirm() {
    ...
    try {
      ...
      setFields((prev) => prev.filter((_, i) => i !== index).map((x, i) => ({ ...x, order_index: i })));
  ...
  async function saveAll() {
    ...
    try {
      ...
      for (const { transform } of candidates) {
        ...
        const { data: d, error: e } = await supabase.from("app_form_fields").upsert(p, { onConflict: "id" }).select();
        if (!e) {
          data = d;
          lastErr = null;
          break;
      ...
      const byKey = new Map();
      for (const row of data || []) byKey.set((row.key || row.field_key) + "|" + (row.context || contextTab), row);
      setFields((prev) =>
        prev.map((f) => {
      ...
      >
        ...
        {loading ? (
          ...
          >
            ...
            >
              <Text style={{ color: colors.tint, fontSize: 16, fontWeight: "600" }}>+ Добавить поле</Text>
            </Pressable>
          </ScrollView>
        )}
  ...
  return (
    ...
    >
      <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
        <Text style={{ color: colors.text, fontSize: 16, fontWeight: "600" }}>{f.label || f.key}</Text>
    ...
    >
      ...
      >
        {saving ? (
          ...
          <Text style={{ color: "#fff", fontWeight: "700", fontSize: 16 }}>Сохранить изменения</Text>
        )}
      </Pressable>
    </View>
  );
