    ...
    <FlatList
      ...
      renderItem={({ item, index: i }) => (
        <View style={[wheelStyles.item, !enabled && { opacity: 0.35 }]}>
          <Text style={[wheelStyles.itemText, i === selIndex && wheelStyles.itemTextActive]}>
            {item}
          </Text>
        </View>
      )}
...
export default function EditUser() {
  ...
  const fetchUser = useCallback(async () => {
    setLoading(true);
    const { data, error } = await supabase.rpc('admin_get_profile_with_email', {
    ...
    if (prof && typeof prof.avatar_url !== 'undefined') {
  ...
  const saveProfileFields = async (rawPhone) => {
    ...
    try {
      ...
      if (resp && resp.ok === false)
        throw new Error(resp?.message || 'Сервер не подтвердил обновление профиля');
      return null;
    } catch (e) {
  ...
  const proceedSave = async (rawPhoneParam) => {
    ...
    try {
      ...
      if (err1) {
        setErr(err1.message || 'Ошибка при сохранении профиля');
        setSaving(false);
        return;
      }
      ...
      if (!res.ok) throw new Error(resp?.message || `HTTP ${res.status} ${res.statusText}`);
      if (resp && resp.ok === false)
        throw new Error(resp?.message || 'Сервер не подтвердил обновление доступа');

      setOk('Сохранено');
      setNewPassword('');
      // Обновляем снапшот и заголовок, и разрешаем выход без предупреждения
      const newFull =
        `${firstName.trim()} ${lastName.trim()}`.replace(/\s+/g, ' ').trim() || 'Без имени';
  ...
  useEffect(() => {
    if (pickerVisible) loadCandidates(pickerQuery);
  }, [pickerVisible, pickerQuery, loadCandidates]);
                    ...
                    {role === 'admin' ? (
                      <View
                        style={[
                          ...
                          {
                            borderColor: roleColor('admin') + '33',
                            backgroundColor: roleColor('admin') + '22',
                          },
                        ]}
                      >
          ...
          >
            ...
            {pickerLoading ? (
              ...
              <FlatList
                ...
                ListEmptyComponent={
                  <Text style={{ textAlign: 'center', color: '#8E8E93', paddingVertical: 16 }}>
                    Ничего не найдено
                  </Text>
                }
              />
