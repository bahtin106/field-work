...
import { fetchFormSchema, buildCustomPayload } from '../lib/settings';
...
export default function CreateOrderScreen() {
 ...
 const styles = useMemo(() => {
    ...
    // Палитра, визуально эквивалентная старому дизайну,
    // но с поддержкой тёмной темы
    const palette = {
      bg: isDark ? '#0B0B0C' : '#f2f2f2',
      card: isDark ? '#151517' : '#fff',
      text: isDark ? '#EAEAEA' : '#111',
      textMuted: isDark ? '#9A9A9E' : '#555',
      border: isDark ? '#2A2A2E' : '#ccc',
      borderSoft: isDark ? '#1E1E22' : '#eee',
      inputBg: isDark ? '#1B1B1E' : '#fff',
      primary: '#007AFF',
      secondary: isDark ? '#2C2C31' : '#d1d1d6',
      destructive: '#FF3B30',
      toggleTrack: isDark ? '#2D2D33' : '#d1d1d6',
      toggleTrackOn: '#FF3B30',
      knob: isDark ? '#1B1B1E' : '#fff',
      icon: isDark ? '#A0A0A6' : '#666',
    };

    return StyleSheet.create({
      container: { flex: 1, backgroundColor: palette.bg },
      scroll: { padding: 16, paddingBottom: 40 },
      pageTitle: { fontSize: 20, fontWeight: '600', marginBottom: 16, textAlign: 'center', color: palette.text },

      card: { backgroundColor: palette.card, borderRadius: 12, padding: 12, borderColor: palette.borderSoft, borderWidth: 1, marginBottom: 12 },
      section: { marginTop: 6, marginBottom: 8, fontWeight: '600', color: palette.text },
      label: { fontWeight: '500', marginBottom: 4, marginTop: 12, color: isDark ? '#E3E3E6' : '#333' },
      input: { borderWidth: 1, borderColor: palette.border, backgroundColor: palette.inputBg, color: palette.text, borderRadius: 10, padding: 10 },
      selectInput: {
        flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center',
        borderWidth: 1, borderColor: palette.border, borderRadius: 10, backgroundColor: palette.inputBg, padding: 12, marginTop: 4
      },
      selectInputText: { fontSize: 16, color: palette.text },

      appButton: { paddingVertical: 12, paddingHorizontal: 16, borderRadius: 8, alignItems: 'center' },
      ...
      toggle: { width: 42, height: 26, borderRadius: 13, backgroundColor: palette.toggleTrack, padding: 2, justifyContent: 'center' },
      toggleOn: { backgroundColor: palette.toggleTrackOn },
      knob: { width: 22, height: 22, borderRadius: 11, backgroundColor: palette.knob, alignSelf: 'flex-start' },
  ...
  const showWarning = (message) => { setWarningMessage(message); setWarningVisible(true); };
  ...
  const scrollToDateField = () => {
    if (scrollRef.current && dateFieldRef.current) {
  ...
  useFocusEffect(useCallback(() => {
    const subscription = BackHandler.addEventListener('hardwareBackPress', () => { setCancelVisible(true); return true; });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        const defaults = [
          { field_key: 'title',  label: 'Название заявки',   type: 'text',  position: 10, required: true },
        ...
        const fields = (Array.isArray(data?.fields) && data.fields.length > 0) ? data.fields : defaults;
        setSchema({ context: 'create', fields });

        const init = {}; for (const f of fields) init[f.field_key] = ''; setForm(init);
      } catch (e) {
  ...
  const getField = useCallback((key) => (schema.fields || []).find((f) => f.field_key === key) || null, [schema]);
  ...
  const AppButton = ({ label, onPress, type = 'primary' }) => (
    <Pressable onPress={onPress}
      style={({ pressed }) => [styles.appButton,
        type === 'destructive' ? styles.btnDestructive :
        type === 'secondary' ? styles.btnSecondary : styles.btnPrimary,
        pressed && { transform: [{ scale: 0.96 }] }]}
    >
      <Text style={[styles.appButtonText,
        type === 'destructive' ? styles.btnDestructiveText :
        type === 'secondary' ? styles.btnSecondaryText : styles.btnPrimaryText]}>
        {label}
      </Text>
    </Pressable>
  );
  ...
  const renderTextInput = (key, placeholder) => {
    ...
    return (
      <View key={key}>
        <Text style={styles.label}>{label}{f?.required ? ' *' : ''}</Text>
        <TextInput
          style={styles.input}
          placeholder={placeholder || label}
          placeholderTextColor={theme.name === 'dark' ? '#7F7F84' : '#999'}
          value={val}
  ...
  const handleSubmit = async () => {
    ...
    if (error) { showWarning(error.message); } else { router.replace('/order-success'); }
  };
      ...
      <SafeAreaView style={{ flex: 1, backgroundColor: styles.container.backgroundColor }} edges={['top', 'left', 'right']}>
          ...
          <ScrollView ref={scrollRef} contentContainerStyle={styles.scroll}
            ...
            <View style={styles.card}>
              ...
              <View style={styles.toggleRow}>
                <Pressable onPress={() => setUrgent(v => !v)} style={[styles.toggle, urgent && styles.toggleOn]}>
              ...
              <View ref={(ref) => { if (ref) dateFieldRef.current = findNodeHandle(ref); }}>
                <Pressable style={styles.selectInput} onPress={() => { setShowDatePicker(true); setTimeout(scrollToDateField, 200); }}>
                  <Text style={styles.selectInputText}>
                    {departureDate
                      ? departureDate.toLocaleDateString('ru-RU', { day: '2-digit', month: 'long', year: 'numeric' })
                      : 'Выберите дату'}
                  </Text>
                  <AntDesign name="calendar" size={16} color={theme.name === 'dark' ? '#A0A0A6' : '#666'} />
                </Pressable>
              </View>
              {showDatePicker && (
                <DateTimePicker value={departureDate || new Date()} mode="date" display="default" minimumDate={new Date()}
                  onChange={(event, selected) => { setShowDatePicker(false); if (selected) setDepartureDate(selected); }} />
              )}

              <View style={[styles.toggleRow, { marginTop: 12 }]}>
                <Pressable onPress={() => setToFeed(prev => { const nv = !prev; if (nv) setAssigneeId(null); return nv; })}
              ...
              <Text style={styles.label}>{(getField('assigned_to')?.label || 'Исполнитель')} {toFeed ? '' : '*'}</Text>
              <Pressable style={[styles.selectInput, toFeed && { opacity: 0.5 }]} onPress={() => setAssigneeModalVisible(true)} disabled={toFeed}>
                <Text style={styles.selectInputText}>
                  {assigneeId ? (() => { const u = users.find((x) => x.id === assigneeId);
                    return [u?.first_name, u?.last_name].filter(Boolean).join(' ') || 'Выбран исполнитель'; })()
                    : (toFeed ? 'В общую ленту' : 'Выберите исполнителя...')}
                </Text>
                <AntDesign name="down" size={16} color={theme.name === 'dark' ? '#A0A0A6' : '#666'} />
              </Pressable>
            </View>

            <View style={{ marginTop: 20 }}>
              <AppButton label="Создать заявку" onPress={handleSubmit} />
            </View>
            <View style={{ marginTop: 12 }}>
              <AppButton label="Отменить" onPress={handleCancelPress} type="secondary" />
            </View>
          </ScrollView>
        </View>
        ...
        <Modal isVisible={assigneeModalVisible} onBackdropPress={() => setAssigneeModalVisible(false)} useNativeDriver backdropOpacity={0.3}>
          <View style={styles.modalContainer}>
            ...
            <Pressable onPress={() => { setToFeed(true); setAssigneeId(null); setAssigneeModalVisible(false); }}
              style={({ pressed }) => [styles.assigneeOption, pressed && { opacity: 0.8 }]}>
              <Text style={styles.assigneeText}>В общую ленту</Text>
            </Pressable>
            <View style={{ height: 8 }} />
            {users.map((user) => (
              <Pressable key={user.id} onPress={() => { setAssigneeId(user.id); setToFeed(false); setAssigneeModalVisible(false); }}
                style={({ pressed }) => [styles.assigneeOption, pressed && { opacity: 0.8 }]}>
