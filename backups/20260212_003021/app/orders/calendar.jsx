...
export default function CalendarScreen() {
  ...
  const expandedCalendarHeight = measuredMonthHeaderHeight + measuredDayNamesHeight + weeksHeight;
  ...
  const onMonthHeaderLayout = useCallback(
    (event) => {
      ...
      setMeasuredMonthHeaderHeight(nextHeight);
    },
    [setMeasuredMonthHeaderHeight],
  );
  ...
  const onWeekdayRowLayout = useCallback(
    (event) => {
      const nextHeight = event?.nativeEvent?.layout?.height;
      if (!Number.isFinite(nextHeight) || nextHeight <= 0) return;
      setMeasuredDayNamesHeight(nextHeight);
    },
  ...
  const onWeekRowLayout = useCallback(
    (event) => {
      const nextHeight = event?.nativeEvent?.layout?.height;
      if (!Number.isFinite(nextHeight) || nextHeight <= 0) return;
      setMeasuredWeekRowHeight(nextHeight);
    },
  ...
  const weeksClipStyle = useAnimatedStyle(() => {
    const split = CALENDAR_GESTURE.MONTH_COLLAPSE_PHASE_SPLIT;
    const fullHeight = measuredWeekRowHeight * actualWeekRows;
    const selectedBottomHeight = measuredWeekRowHeight * (selectedWeekIndex + 1);
    const height = interpolate(
      stageAtoBProgress.value,
      [0, split, 1],
      [fullHeight, selectedBottomHeight, measuredWeekRowHeight],
      Extrapolate.CLAMP,
    );
    return { height, overflow: 'hidden' };
  }, [actualWeekRows, measuredWeekRowHeight, selectedWeekIndex]);

  const weeksTranslateStyle = useAnimatedStyle(() => {
    const split = CALENDAR_GESTURE.MONTH_COLLAPSE_PHASE_SPLIT;
    const secondPhaseProgress = interpolate(
      stageAtoBProgress.value,
      [split, 1],
      [0, 1],
      Extrapolate.CLAMP,
    );
    const weekOffset = -selectedWeekIndex * measuredWeekRowHeight * secondPhaseProgress;
  ...
  const collapseDuration = theme.timings.panelToggleMs;
  ...
  const calendarGesture = Gesture.Pan()
    ...
    .onEnd((event) => {
      ...
      if (Math.abs(velocityY) > threshold) {
        ...
        target = progress > CALENDAR_GESTURE.SNAP_PROGRESS_THRESHOLD ? total : 0;
      }
  ...
  const ordersListNativeGesture = Gesture.Native();
  const ordersPanGesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
      ...
      // Up: when collapsed and list can still scroll, keep control in list.
      if (ty < 0 && isFullyCollapsed && listIsAwayFromTop) return;
  ...
  const calendarIndex = useMemo(() => {
    ...
    return { byDate, byMonth, marksBase, countByDate };
  ...
  const selectedMonthOrders = useMemo(() => {
    if (!isCollapsed || !selectedYearMonth) return [];
    return calendarIndex.byMonth[selectedYearMonth] ?? [];
  }, [isCollapsed, selectedYearMonth, calendarIndex]);

  const displayedOrders = useMemo(
    () => (isCollapsed ? selectedMonthOrders : selectedDateOrders),
    [isCollapsed, selectedMonthOrders, selectedDateOrders],
  );
      ...
      <View style={styles.container}>
        {viewMode === 'month' ? (
          <>
                ...
                <View style={[styles.calendarContent]}>
                  <AnimatedFlatList
                    ...
                    renderItem={({ item: monthDate }) => {
                      ...
                      const fixedWeekRows = CALENDAR_LAYOUT.FIXED_WEEK_ROWS;
            ...
            <GestureDetector gesture={ordersPanGesture}>
                  ...
                  <View style={{ flex: 1 }}>
                    ...
                    <GestureDetector gesture={ordersListNativeGesture}>
                      <FlatList
                        ...
                        extraData={{ selectedDate, count: displayedOrders.length, isCollapsed }}
