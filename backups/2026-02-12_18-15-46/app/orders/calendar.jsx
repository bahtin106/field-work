...
export default function CalendarScreen() {
  ...
  const [lockedTransitionWeekRows, setLockedTransitionWeekRows] = useState(6);
  const isMonthTransitioningRef = useRef(false);
  ...
  const [settledMonthIndex, setSettledMonthIndex] = useState(MONTH_LIST_MIDDLE_INDEX);
  const deferInitialMeasureRef = useRef(true);
  const weekRowMeasuredRef = useRef(false);
  const initialMonthRef = useRef(startOfMonth(new Date()));
  const monthMomentumActiveRef = useRef(false);
  ...
  const weekRowsByIndex = useMemo(
    () => monthWeeksByIndex.map((weeks) => weeks.length),
    [monthWeeksByIndex],
  );
  ...
  const renderedWeekRows = isMonthTransitioning ? lockedTransitionWeekRows : actualWeekRows;
  useEffect(() => {
    if (isMonthTransitioning) return;
    setLockedTransitionWeekRows(actualWeekRows);
  }, [actualWeekRows, isMonthTransitioning]);
  ...
  const weeksHeight = measuredWeekRowHeight * renderedWeekRows;
  ...
  const collapseGestureTravel = useMemo(
    () => Math.max(1, resolvedMonthHeaderHeight + measuredWeekRowHeight * 5),
    [measuredWeekRowHeight, resolvedMonthHeaderHeight],
  );
  ...
  const handlePageChange = useCallback(
    (pageIndex) => {
      ...
      setSettledMonthIndex(pageIndex);
  ...
  const commitVisibleMonthIndex = useCallback(
    (pageIndex) => {
      visibleMonthIndexRef.current = pageIndex;
      visibleMonthIndex.value = pageIndex;
      monthScrollX.value = layoutMetrics.cardWidth * pageIndex;
      handlePageChange(pageIndex);
      setIsMonthTransitioning(false);
    },
    [handlePageChange, layoutMetrics.cardWidth, monthScrollX, visibleMonthIndex],
  );
  ...
  useEffect(() => {
    ...
    setSettledMonthIndex(targetIndex);
    setIsMonthTransitioning(false);
  ...
  useEffect(() => {
    ...
    weekRowMeasuredRef.current = false;
  ...
  const onWeekRowLayout = useCallback(
    (event) => {
      if (deferInitialMeasureRef.current) return;
      if (weekRowMeasuredRef.current) return;
      const nextHeight = event?.nativeEvent?.layout?.height;
      if (!Number.isFinite(nextHeight) || nextHeight <= 0) return;
      setMeasuredWeekRowHeight((prev) =>
        Math.abs(prev - nextHeight) <= CALENDAR_LAYOUT.MEASURE_DELTA_EPSILON ? prev : nextHeight,
      );
      weekRowMeasuredRef.current = true;
    },
  ...
  useEffect(() => {
    isMonthTransitioningRef.current = isMonthTransitioning;
  }, [isMonthTransitioning]);
  const startMonthTransition = useCallback(() => {
    if (isMonthTransitioningRef.current) return;
    const index = visibleMonthIndexRef.current;
    const currentRows = weekRowsByIndex[index] ?? actualWeekRows;
    setLockedTransitionWeekRows(currentRows);
    setIsMonthTransitioning(true);
  }, [actualWeekRows, weekRowsByIndex]);
  const ordersVisibility = useSharedValue(1);
  useEffect(() => {
    ordersVisibility.value = withTiming(isMonthTransitioning ? 0 : 1, {
      duration: isMonthTransitioning ? 90 : 180,
    });
  }, [isMonthTransitioning, ordersVisibility]);
      ...
      height: (() => {
        ...
        if (progress <= 0.001) {
          return (
            ...
            measuredWeekRowHeight * renderedWeekRows
          );
        }
  ...
  const eventCountAnimatedStyle = useAnimatedStyle(() => {
    ...
    return {
      ...
      transform: [{ scale: interpolate(progress, [0, 1], [1, 0.22], Extrapolate.CLAMP) }],
    };
  });
  const eventDotAnimatedStyle = useAnimatedStyle(() => {
    const progress = stageAtoBProgress.value;
    const leadProgress = Math.min(Math.max(progress + 0.34, 0), 1);
    return {
      opacity: interpolate(progress, [0, 0.08, 1], [0, 0, 1], Extrapolate.CLAMP),
      transform: [{ scale: interpolate(leadProgress, [0, 1], [0.22, 1], Extrapolate.CLAMP) }],
    };
  });
  ...
  const weeksClipStyle = useAnimatedStyle(() => {
    const progress = stageAtoBProgress.value;
    if (progress <= 0.001) {
      return { height: measuredWeekRowHeight * renderedWeekRows, overflow: 'hidden' };
    }
  ...
  const scrollToMonthByOffset = useCallback(
    ...
    [dynamicMonths.length, startMonthTransition],
  );
  ...
  const horizontalFailOffsetY = gestureFailOffsetY + 10;
  ...
  const verticalActiveOffsetY = Math.max(4, gestureActiveOffsetY - 6);
  const verticalLockDistance = Math.max(4, directionLockDistance - 5);
  const verticalDirectionLockRatio = Math.max(1.2, directionLockRatio - 0.15);
  const calendarGestureLock = useSharedValue(0); // 0 unknown, 1 horizontal, 2 vertical
  const ordersPanLock = useSharedValue(0); // 0 unknown, 1 horizontal, 2 vertical
  ...
  const calendarGesture = Gesture.Pan()
    ...
    .activeOffsetY([-verticalActiveOffsetY, verticalActiveOffsetY])
    ...
    .onUpdate((event) => {
      ...
      if (calendarGestureLock.value === 0 && (absTx > verticalLockDistance || absTy > verticalLockDistance)) {
        if (absTy > absTx * verticalDirectionLockRatio) calendarGestureLock.value = 2;
        else if (absTx > absTy * verticalDirectionLockRatio) calendarGestureLock.value = 1;
      }
      ...
      const startProgress = stageOneDistanceSafe > 0 ? gestureStart.value / stageOneDistanceSafe : 0;
      const progressDelta = -ty / collapseGestureTravel;
      const nextProgress = clamp(startProgress + progressDelta, 0, 1);
      const safeMax = Number.isFinite(stageOneDistanceSafe) ? stageOneDistanceSafe : 0;
      collapseTranslate.value = nextProgress * safeMax;
  ...
  const ordersMonthGesture = Gesture.Pan()
    ...
    .failOffsetY([-horizontalFailOffsetY, horizontalFailOffsetY])
    .onEnd((event) => {
      'worklet';
      if (isSnappingShared.value) return;
  ...
  const ordersPanGesture = Gesture.Pan()
    ...
    .onUpdate((event) => {
      ...
      if (ordersPanLock.value === 0 && (absTx > verticalLockDistance || absTy > verticalLockDistance)) {
        if (absTy > absTx * verticalDirectionLockRatio) ordersPanLock.value = 2;
        else if (absTx > absTy * verticalDirectionLockRatio) ordersPanLock.value = 1;
      }
      if (ordersPanLock.value === 1) return;
  ...
  const settledMonthDate = dynamicMonths[settledMonthIndex] ?? currentMonth;
  const selectedMonthKey = selectedDate ? selectedDate.slice(0, 7) : '';
  const settledMonthKey = format(settledMonthDate, 'yyyy-MM');
  const settledPageDateKey =
    settledMonthKey === selectedMonthKey
      ? selectedDate
      : format(startOfMonth(settledMonthDate), 'yyyy-MM-dd');
  const displayedOrders = useMemo(
    () => (settledPageDateKey ? calendarIndex.byDate[settledPageDateKey] ?? [] : []),
    [calendarIndex.byDate, settledPageDateKey],
  );
  ...
  const ordersContentAnimatedStyle = useAnimatedStyle(() => ({
    opacity: ordersVisibility.value,
  }));
  const ordersContentContainerStyle = useMemo(
    () => ({
      paddingHorizontal: theme.spacing.md,
              ...
              <Animated.View style={[calendarContentStyle]}>
                <View style={[styles.calendarContent]}>
                  ...
                  <AnimatedFlatList
                    ...
                    onScrollBeginDrag={() => {
                      monthMomentumActiveRef.current = false;
                      startMonthTransition();
                    }}
                    onMomentumScrollBegin={() => {
                      monthMomentumActiveRef.current = true;
                      startMonthTransition();
                    }}
                    onScrollEndDrag={(event) => {
                      const vx = Math.abs(Number(event?.nativeEvent?.velocity?.x) || 0);
                      if (monthMomentumActiveRef.current || vx > 0.05) return;
            ...
            <GestureDetector gesture={ordersCombinedGesture}>
              <View style={{ flex: 1, width: '100%' }}>
                ...
                <View
                  ...
                  pointerEvents={isMonthTransitioning ? 'none' : 'auto'}
                >
                  <Animated.View
                    style={[
                      {
                    ...
                    >
                      <View style={styles.ordersHeader}>
                        <Text style={styles.ordersTitle}>
                          Заявки на {formatDayLabel(settledPageDateKey)}
                        </Text>
                      ...
                      <GestureDetector gesture={ordersListNativeGesture}>
                        <FlatList
                          ...
                          contentContainerStyle={ordersContentContainerStyle}
                          style={{ flex: 1 }}
                          scrollEnabled={!isMonthTransitioning && isCollapsed && !isSnapping}
                          bounces={false}
                          scrollEventThrottle={16}
                          onScroll={(event) => {
