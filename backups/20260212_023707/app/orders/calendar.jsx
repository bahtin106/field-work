...
export default function CalendarScreen() {
  ...
  const pendingMonthIndexRef = useRef(null);
  ...
  const commitVisibleMonthIndex = useCallback(
    (pageIndex) => {
      ...
      // Final momentum position is source-of-truth: drop delayed queued hops
      // to avoid post-settle month flicker.
      pendingMonthIndexRef.current = null;
      isMonthTransitioningRef.current = false;
    },
  ...
  const scrollToMonthByOffset = useCallback(
    (offset) => {
      ...
      if (isMonthTransitioningRef.current) {
        pendingMonthIndexRef.current = nextIndex;
        return;
      }
      isMonthTransitioningRef.current = true;
      pendingMonthIndexRef.current = null;
  ...
  const ordersRenderNeighbors = Math.max(2, CALENDAR_LAYOUT.MONTH_RENDER_NEIGHBORS);
