...
export default function CalendarScreen() {
  ...
  const ordersRenderIndices = useMemo(() => {
    const maxIndex = Math.max(0, dynamicMonths.length - 1);
    const start = clamp(visibleMonthRenderIndex - 1, 0, Math.max(0, maxIndex - 2));
    const indices = [start, start + 1, start + 2].filter((idx) => idx >= 0 && idx <= maxIndex);
    return indices;
  }, [dynamicMonths.length, visibleMonthRenderIndex]);
  const ordersRenderStartIndex = ordersRenderIndices[0] ?? 0;
  ...
  const ordersInnerStyle = useAnimatedStyle(
    () => ({
      transform: [
        {
          translateX: -(monthScrollX.value - layoutMetrics.cardWidth * ordersRenderStartIndex),
        },
      ],
    }),
    [layoutMetrics.cardWidth, ordersRenderStartIndex],
  );
                ...
                >
                  <Animated.View
                    style={[
                      {
                        flexDirection: 'row',
                        width: layoutMetrics.cardWidth * ordersRenderIndices.length,
                        flex: 1,
                      },
                      ordersInnerStyle,
                    ]}
                  >
                    {ordersRenderIndices.map((index) => {
                      const monthDate = dynamicMonths[index];
                      if (!monthDate) {
                        return (
                          <View
                            key={`orders-missing-${index}`}
                      ...
                      const shouldRenderList = isActivePage;
