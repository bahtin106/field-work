...
alter table public.company_finance_rules
  add column if not exists apply_to_existing boolean not null default false;

update public.company_finance_rules
   set percent_base = 'gross_after_discount'
 where percent_base not in ('base_price', 'gross_before_discount', 'gross_after_discount', 'income_total');
...
begin
  ...
  loop
    ...
    if v_fact = 'payment_method' then
      ...
      if v_operator <> 'eq' or v_actual <> v_expected then
        return false;
      end if;
    elsif v_fact = 'payment_status' then
...
begin
  ...
  select coalesce(array_agg(r.id), '{}'::uuid[])
   ...
   where r.company_id = v_order.company_id
     ...
     and (r.apply_to_existing = true or r.created_at <= v_order.created_at);
