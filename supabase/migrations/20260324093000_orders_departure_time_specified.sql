-- Explicit flag for departure time presence.
-- Separates "date exists" from "time explicitly selected", so 00:00 can be intentional.

alter table if exists public.orders
  add column if not exists departure_time_specified boolean not null default false;

-- Backfill existing rows: if time part is not midnight, treat as explicitly specified.
update public.orders
set departure_time_specified = true
where time_window_start is not null
  and (
    extract(hour from time_window_start) <> 0
    or extract(minute from time_window_start) <> 0
    or extract(second from time_window_start) <> 0
  );
...
begin
  ...
  update public.orders o
     set title = case when p_patch ? 'title' then nullif(p_patch->>'title', '') else o.title end,
         ...
         departure_time_specified = case when p_patch ? 'departure_time_specified' then coalesce((p_patch->>'departure_time_specified')::boolean, false) else o.departure_time_specified end,
