...
set comment = nullif(trim(coalesce(comment, '')), '')
where comment is distinct from nullif(trim(coalesce(comment, '')), '');
...
begin
  if not exists (
    select 1
      from pg_constraint
     where conname = 'clients_comment_length_chk'
...
begin
  ...
  new.comment := nullif(left(btrim(coalesce(new.comment, '')), 500), '');
...
comment on column public.clients.comment is
  'Client note for internal communication.';
