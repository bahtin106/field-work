...
-- `new` and `done` remain system status keys, but their display names are
-- company-specific and may be changed from the order board.
...
begin
  ...
  -- `new` and `done` preserve their system identity. Their name is a display
  -- value, while color and ordering remain protected for now.
  if old.status_key in ('new', 'done')
     and (
       new.color is distinct from old.color
