...
begin
  ...
  loop
    ...
    if v_fact in ('payment_method', 'payment_status') then
      ...
      if v_operator <> 'eq' or jsonb_typeof(v_value) <> 'string' then
        return false;
      end if;
        ...
        if exists (
          select 1
           ...
           where jsonb_typeof(elem.value) <> 'string'
              or lower(coalesce(elem.value #>> '{}', '')) !~ v_uuid_re
        ) then
          return false;
        end if;
      else
    ...
    elsif v_fact in ('price', 'base_price', 'start_price', 'gross_before_discount', 'gross_after_discount', 'income_total') then
...
after insert or update of start_price, assigned_to, company_id, payment_method, payment_status, work_type_id
on public.orders
