-- Separate departure time from departure date.
-- Keeps date-only and time-only scenarios independent.

alter table if exists public.orders
  add column if not exists departure_time time without time zone;

-- Backfill from legacy datetime (non-midnight time part).
update public.orders
set departure_time = make_time(
  extract(hour from time_window_start)::int,
  extract(minute from time_window_start)::int,
  0
)
...
-- Cleanup temporary explicit flag introduced previously.
alter table if exists public.orders
  drop column if exists departure_time_specified;
