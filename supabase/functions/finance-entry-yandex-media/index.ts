...
const FINANCE_ROOT_DIR = '\u0424\u0438\u043d\u0430\u043d\u0441\u044b';
...
function buildFinanceEntryLabel(entry: {
  id: string;
  title?: string | null;
}) {
  const shortId = String(entry.id || '').slice(0, 8) || 'entry';
  const base = String(entry.title || '').trim() || `\u0441\u0442\u0430\u0442\u044c\u044f_${shortId}`;
  const safeBase = sanitizePathSegment(base, `\u0441\u0442\u0430\u0442\u044c\u044f_${shortId}`);
  return `${safeBase}_${shortId}`;
}
...
function buildFinanceEntryMediaFolder(
  ...
  const financeDir = buildFinanceEntryLabel(financeEntry);
  return `${root}/${companyDir}/${ORDERS_ROOT_DIR}/${monthDir}/${orderDir}/${FINANCE_ROOT_DIR}/${financeDir}`;
}
...
async function getCallerAndFinanceEntryContext(
  ...
  if (String(entry.company_id) !== String(profile.company_id)) throw new Error('Forbidden');
...
async function appendFinanceEntryPhotoUrlAtomic(
  ...
  const urls = Array.isArray(row?.photo_urls) ? row.photo_urls.map((x: unknown) => String(x || '')) : null;
  return {
    photo_urls: urls,
...
async function removeFinanceEntryPhotoUrlAtomic(
  ...
  const row = Array.isArray(data) ? data[0] : data;
  const urls = Array.isArray(row?.photo_urls) ? row.photo_urls.map((x: unknown) => String(x || '')) : [];
  return {
    photo_urls: urls,
    updated_at: row?.updated_at ? String(row.updated_at) : null,
  };
...
export async function handleFinanceEntryYandexMediaRequest(req: Request) {
  ...
  try {
    ...
    if (action === 'commit_upload') {
      ...
      const { error: mapErr } = await admin.from('finance_entry_media_external_map').upsert(
        {
          ...
          finance_entry_id: ctx.financeEntry.id,
          provider: 'yandex_disk',
      ...
      const atomicResult = await appendFinanceEntryPhotoUrlAtomic(
        admin,
        ctx.financeEntry.id,
        ctx.companyId,
        sourceUrl,
      ...
      return json(200, {
        ...
        display_url: displayUrl,
        provider: 'yandex_disk',
        photo_urls: atomicResult.photo_urls || [],
        finance_entry_updated_at: atomicResult.updated_at,
      });
    }
    ...
    if (action === 'inspect_urls') {
      ...
      let financeEntryCandidates: Array<{ id: number; source_url: string; external_path: string }> = [];
      if (accessToken) {
        ...
        financeEntryCandidates = candidates || [];
      }
      ...
      for (const sourceUrl of urls) {
        ...
        if (!externalPath) {
          ...
          if (needle && accessToken) {
            mapRow =
              financeEntryCandidates.find((row) => canonicalUrl(String(row.source_url || '')) === needle) || null;
            if (!mapRow) {
              for (const candidate of financeEntryCandidates) {
    ...
    const status = lowered.includes('unauthorized')
      ...
      : lowered.includes('forbidden')
        ...
        : lowered.includes('missing') || lowered.includes('invalid')
          ? 400
          : 500;
    console.error('[finance-entry-yandex-media]', status, message);
