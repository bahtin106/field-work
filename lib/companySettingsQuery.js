...
let preferSelectWithoutOrderHistory = false;
...
async function selectCompanySettings(companyId, selectClause, signal = undefined) {
  let query = supabase
    .from('companies')
    .select(selectClause)
    .eq('id', companyId)
...
export async function fetchCompanySettingsByCompanyId(companyId, signal = undefined) {
  if (!companyId) return null;
  if (preferLegacySelect) {
    return selectCompanySettings(companyId, LEGACY_SELECT, signal);
  }
  if (!preferSelectWithoutFeedFields) {
    try {
      return await selectCompanySettings(companyId, PREFERRED_SELECT, signal);
    } catch (error) {
  ...
  if (!preferSelectWithoutOrderHistory) {
    try {
      return await selectCompanySettings(companyId, ORDER_HISTORY_SELECT, signal);
    } catch (error) {
      if (!isSchemaMismatchError(error)) throw error;
      preferSelectWithoutOrderHistory = true;
    }
  }

  try {
    return await selectCompanySettings(companyId, EXTENDED_SELECT, signal);
  } catch (error) {
    if (!isSchemaMismatchError(error)) throw error;
