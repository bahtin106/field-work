...
import { resolveAppLocale, resolveDateFnsLocale } from './localeFormatting';
...
export function formatDateWithOptionalTime(dateIso, useDepartureTime = false) {
  ...
  try {
    ...
    if (useDepartureTime) {
      return format(date, 'd MMMM yyyy, HH:mm', { locale: resolveDateFnsLocale() });
    }

    // Без времени: "13 декабря 2025"
    return format(date, 'd MMMM yyyy', { locale: resolveDateFnsLocale() });
  } catch {
    return '';
  }
}
...
export function getTimeOnly(dateIso) {
  ...
  try {
    ...
    return date.toLocaleTimeString(resolveAppLocale(), { hour: '2-digit', minute: '2-digit' });
  } catch {
    return '';
  }
}
...
export function formatDateShortWithOptionalTime(dateIso, useDepartureTime = false) {
  ...
  try {
    ...
    if (useDepartureTime) {
      return format(date, 'd MMM, HH:mm', { locale: resolveDateFnsLocale() });
    }

    // Без времени: "13 дек"
    return format(date, 'd MMM', { locale: resolveDateFnsLocale() });
  } catch {
    return '';
  }
}
