...
export async function objectMediaStorage(action, payload = {}) {
  ...
  const { data, error } = await supabase.functions.invoke('object-media-storage', {
    headers: { Authorization: `Bearer ${token}` },
    body: { action, ...payload },
  });

  if (error) {
    throw new Error(await readInvokeErrorMessage(error, 'Object media storage action failed'));
  }
  if (!data?.success) {
    throw new Error(buildErrorMessage(data, 'Object media storage action failed'));
  }
  return data;
}
