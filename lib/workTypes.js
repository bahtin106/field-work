...
export const WORK_TYPES_NETWORK_DEFERRED_CODE = 'WORK_TYPES_NETWORK_DEFERRED';
...
function readWorkTypesCache(cacheKey) {
  ...
  if (Date.now() - cacheEntry.storedAt > WORK_TYPES_DISK_CACHE_MAX_AGE_MS) {
...
function writeWorkTypesCache(cacheKey, payload, storedAt = Date.now()) {
...
async function selectWorkTypes(companyId, signal) {
...
async function fetchWorkTypesFromNetwork(companyId, includeDisabled, cacheKey, upstreamSignal = undefined) {
  const result = await withReadDeadline(
    (signal) =>
      Promise.all([
        supabase
          .from('companies')
          .select('id,use_work_types')
          .eq('id', companyId)
          .single()
          .abortSignal(signal),
        selectWorkTypes(companyId, signal),
      ]),
    { label: 'Work types', signal: upstreamSignal },
  );
  const [{ data: company, error: ce }, typesResult] = result;
...
function refreshWorkTypesInBackground(companyId, includeDisabled, cacheKey) {
  ...
  if (!network.isOnline || network.isPoorConnection) return;
...
export async function fetchWorkTypes(companyId, options = {}) {
  ...
  if (!forceRefresh) {
    ...
    if (cached) {
      if (Date.now() - cached.storedAt > WORK_TYPES_CACHE_TTL_MS) {
        refreshWorkTypesInBackground(companyId, includeDisabled, cacheKey);
      }
      return { useWorkTypes: cached.useWorkTypes, types: cached.types };
    }
    ...
    if (diskCached) {
      ...
      writeWorkTypesCache(cacheKey, payload, diskCached.storedAt);
  ...
  // A cache miss must not occupy the foreground socket on EDGE/offline. The
  // caller can retry when network quality becomes healthy; existing cache is
  // still returned above regardless of its freshness.
  if (options?.deferNetworkWhenConstrained && !canRunDeferredNetworkWork()) {
    const error = new Error('Work types network refresh deferred');
    error.code = WORK_TYPES_NETWORK_DEFERRED_CODE;
    throw error;
  }

  return fetchWorkTypesFromNetwork(companyId, includeDisabled, cacheKey, signal);
}
...
export async function deleteWorkType(companyId, id) {
  ...
  // Deleting a type also clears it from requests. Keep this in one server
  // transaction so a type that is protected by an active finance scheme does
  // not get detached from requests before the deletion is rejected.
  const { data, error } = await supabase.rpc('delete_work_type_v2', {
    p_company_id: companyId,
    p_work_type_id: id,
  });
  if (error) {
    if (isRlsDeniedError(error)) throw new Error('work_types_forbidden_delete');
    throw error;
  }
  if (data !== true) throw new Error('work_types_delete_no_rows');
  invalidateWorkTypesCache(companyId);
}
