...
export function formatPhoneVisibilitySummary(rules, t) {
  const normalized = normalizePhoneVisibilityRules(rules);
  if (normalized.start.type === 'never') return t('settings_phone_rule_summary_never', 'Никогда');
  if (normalized.start.type === 'always' && normalized.stop.type === 'never') {
    return t('settings_phone_rule_summary_always', 'Всегда');
  }
  const start = formatRuleSummary(normalized.start, t, 'start');
  const stop = normalized.stop.type === 'never'
    ? t('settings_phone_rule_summary_no_stop', 'без авто-скрытия')
...
function formatRuleSummary(rule, t, scope) {
  if (rule.type === 'always') return t('settings_phone_rule_summary_always', 'Всегда');
  if (rule.type === 'never') return t('settings_phone_rule_summary_never', 'Никогда');
  ...
  if (rule.type === 'status') {
    return rule.offsetMins > 0
      ? t(
          ...
          scope === 'start' ? 'после статуса "{status}" + {delay}' : 'скрыть после статуса "{status}" + {delay}',
        )
      ...
      : t(
          ...
          scope === 'start' ? 'при статусе "{status}"' : 'скрыть при статусе "{status}"',
        ).replace('{status}', statusLabel);
  }
  if (rule.type === 'time_before_departure' && rule.offsetMins === 0) {
    return t('phone_visibility_summary_at_departure', 'в момент выезда');
  }
  if (rule.type === 'time_after_departure' && rule.offsetMins === 0) {
    return t('phone_visibility_summary_immediately_after_departure', 'сразу после выезда');
  }
  return rule.type === 'time_before_departure'
    ? t('phone_visibility_summary_before_departure', 'за {delay} до выезда').replace('{delay}', formatDuration(rule.offsetMins, t))
    : t('phone_visibility_summary_after_departure', 'через {delay} после выезда').replace('{delay}', formatDuration(rule.offsetMins, t));
}

function formatDuration(mins, t) {
  const value = toNonNegativeInteger(mins, 0);
  if (value === 0) return t('phone_visibility_delay_immediately', 'сразу');
  if (value % 1440 === 0) return `${value / 1440} ${t('unit_days_short', 'дн.')}`;
  if (value % 60 === 0) return `${value / 60} ${t('unit_hours_short', 'ч')}`;
  return `${value} ${t('unit_minutes_short', 'мин')}`;
}
