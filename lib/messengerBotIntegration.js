...
function fallbackMessage(provider) {
  return provider === 'max'
    ? 'Не удалось загрузить настройки MAX-бота.'
...
function humanizeMessengerBotError(provider, message) {
  ...
  if (lowered.includes('forbidden') || lowered.includes('access denied')) {
    return `Недостаточно прав для настройки ${provider === 'max' ? 'MAX' : 'Telegram'}-бота.`;
  }
  if (lowered.includes('company not found')) {
    return 'Компания не найдена. Обновите профиль или войдите снова.';
  }
  if (
    ...
    return 'Временная ошибка загрузки настроек. Попробуйте открыть страницу еще раз.';
  }
