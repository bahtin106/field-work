...
export function isValidHumanName(name) {
  ...
  const { MIN_LENGTH, MAX_LENGTH, PATTERN } = AUTH_CONSTRAINTS.NAME;
...
export function isValidPassword(password) {
  return getPasswordValidationErrors(password).valid;
}

/**
 * Валидирует пароль для ВХОДА (без минимальной длины - пользователь должен войти с любым паролем)
 * @param {string} password
 * @returns {boolean} true если пароль валиден для входа
 */
...
export function hasInvalidPasswordChars(password) {
  if (typeof password !== 'string') return false;
  AUTH_CONSTRAINTS.PASSWORD.INVALID_CHARS_PATTERN.lastIndex = 0;
...
/**
 * Возвращает состояние критериев сложности пароля для UI-подсказок.
 * @param {string} password
 * @returns {{minLength: boolean, hasUpper: boolean, hasLower: boolean, hasDigit: boolean}}
 */
export function getPasswordStrengthChecks(password) {
  const value = typeof password === 'string' ? password : '';
  return {
    minLength: value.length >= AUTH_CONSTRAINTS.PASSWORD.MIN_LENGTH,
