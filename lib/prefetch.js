  ...
  async start() {
    ...
    const dynamicTasks = PREFETCH_TASKS.map((task) => {
      ...
      if (task.queryKey[0] === 'counts' && task.queryKey[1] === 'my' && uid) {
        ...
        if (__DEV__)
          console.warn(
            `[Prefetch] Updated task queryKey: ${task.queryKey.join(':')} → ${updatedTask.queryKey.join(':')}`,
          );
        return updatedTask;
      }
      return task;
    ...
    if (__DEV__) console.warn('[Prefetch] Completed - all data preloaded! 🚀');
    this.isRunning = false;
  }
  ...
  async prefetchTask(task) {
    try {
      ...
      if (__DEV__) {
        ...
        console.warn(`[Prefetch] ✓ Loaded: ${keyStr.substring(0, 150)}`);
      }
    } catch (error) {
