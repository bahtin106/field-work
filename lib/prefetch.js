 ...
 * 2. Использует общие queryKey TanStack Query (единый кэш)
...
// КРИТИЧНО: queryKey должны совпадать с ключами активных useQuery/useInfiniteQuery
const PREFETCH_TASKS = [
  ...
  // HIGH - настройки компании (быстрые данные)
  {
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from('companies')
        .select(
          'name, timezone, use_departure_time, worker_phone_mode, worker_phone_window_before_mins, worker_phone_window_after_mins, media_provider',
        )
  ...
  // HIGH - настройки уведомлений
  {
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        ...
        .select('allow, new_orders, feed_orders, reminders, reminder_delay_minutes, quiet_start, quiet_end')
        .eq('user_id', user.id)
        .maybeSingle();

      if (!error) return data;
      if (!isMissingReminderDelayColumnError(error)) return null;
  ...
  // HIGH - календарь: заказы за последние ~3 месяца для быстрого открытия экрана календаря
  {
    queryKey: ['calendar-orders'],
    priority: PRIORITY.HIGH,
    delay: 0,
    queryFn: async (roleOverride, uidOverride) => {
      const {
        data: { user },
      } = await supabase.auth.getUser();
      ...
      if (!roleOverride) {
        try {
          ...
          // silent fallback to worker
        }
      }

      const sinceIso = new Date(new Date().setMonth(new Date().getMonth() - 3)).toISOString();
      let query = supabase
        .from('orders_secure_v2')
        .select('*')
        .or(`time_window_start.gte.${sinceIso},created_at.gte.${sinceIso}`)
        .order('time_window_start', { ascending: false })
...
class PrefetchManager {
  ...
  /**
   * Prefetch a single page task by key.
   * Supports either:
   * - legacy string key (first segment of queryKey), or
   * - full queryKey joined by ":".
   */
  async prefetchPage(pageKey) {
    if (!pageKey) return;
    const task = PREFETCH_TASKS.find((t) => {
      const key = Array.isArray(t.queryKey) ? t.queryKey : [t.queryKey];
      const first = String(key[0] || '');
      const joined = key.map((part) => String(part)).join(':');
      return first === String(pageKey) || joined === String(pageKey);
    });
    if (!task) return;
