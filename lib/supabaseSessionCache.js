...
export async function getCachedSupabaseAuthContext(expectedUserId = null) {
  const expected = normalizeUserId(expectedUserId);
  const accessToken = await getCachedSupabaseAccessToken(expected);
  const userId = cachedAccessToken === accessToken
    ? normalizeUserId(cachedAccessTokenUserId)
    : '';

  if (
    !accessToken ||
    !userId ||
    !isExpectedUser(userId, expected) ||
    (observedAuthReady && userId !== observedAuthUserId)
  ) {
