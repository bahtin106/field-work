...
export async function savePushToken(userId, token, platform, options = {}) {
  try {
    const {
      data: { session },
    ...
    if (!sessionUserId || sessionUserId !== String(userId || '').trim()) {
      throw new Error('Push token sync belongs to a stale auth session');
    }
    ...
    const body = {
      ...
      enable_notifications: options?.enableNotifications !== false,
    };
...
/** Save the current user's notification event preferences through the trusted service path. */
export async function saveNotificationPreferences(patch) {
  ...
  const payload = Object.entries(patch || {}).reduce((result, [key, value]) => {
  ...
  try {
    ...
    const { data, error } = await supabase.functions.invoke('push-token-sync', {
      headers: { Authorization: `Bearer ${accessToken}` },
      body: { action: 'set_prefs', prefs: payload },
    });
    if (!error && isSuccessfulFunctionPayload(data)) return true;
    if (error) throw new Error(await extractInvokeError(error, 'push-token-sync set_prefs failed'));
    throw new Error(normalizeErrMessage(data, 'push-token-sync set_prefs failed'));
  } catch (error) {
    throw new Error(normalizeErrMessage(error, 'push-token-sync set_prefs failed'));
  }
}
...
export async function deletePushToken(userId, options = {}) {
  ...
  const providedAccessToken = String(options?.accessToken || '').trim();
  const deviceId = await getDeviceId();
  ...
  try {
    ...
    if (!accessToken) {
      ...
      accessToken = session?.access_token ? String(session.access_token) : '';
    }
    if (accessToken) {
      const { data, error } = await supabase.functions.invoke('push-token-sync', {
        headers: { Authorization: `Bearer ${accessToken}` },
  ...
  if (deviceId) fallbackFilters.push({ column: 'device_id', value: deviceId });
  if (pushToken) fallbackFilters.push({ column: 'token', value: pushToken });
  if (!fallbackFilters.length) fallbackFilters.push(null);

  for (const filter of fallbackFilters) {
    let fallbackDelete = supabase.from('push_tokens').delete().eq('user_id', userId);
    if (filter) fallbackDelete = fallbackDelete.eq(filter.column, filter.value);
