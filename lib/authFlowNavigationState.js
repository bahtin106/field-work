...
function getPersistedPublicAuthRoute(route) {
  // An untouched registration form is transient UI state. Persisting it makes
  // a cold unauthenticated launch skip the expected login screen. Registration
  // code and password/email verification screens remain resumable.
  return route === REGISTER_ROUTE ? LOGIN_ROUTE : route;
}
...
export function rememberPublicAuthRoute(input, options = {}) {
  ...
  AsyncStorage.setItem(
    PUBLIC_AUTH_ROUTE_STORAGE_KEY,
    getPersistedPublicAuthRoute(route),
  ).catch(() => {});
  return route;
}
...
export async function hydratePublicAuthRoute() {
  ...
  const storageHydration = (async () => {
    try {
      ...
      if (storedRoute) {
        const hydratedRoute = getPersistedPublicAuthRoute(storedRoute);
        lastPublicAuthRoute = hydratedRoute;
        if (hydratedRoute !== storedRoute) {
          await AsyncStorage.setItem(PUBLIC_AUTH_ROUTE_STORAGE_KEY, hydratedRoute);
        }
