...
import { loadNotificationsModule } from './notificationsCompat';
...
async function getPushTokenIfAlreadyGranted() {
  try {
    ...
    if (!Notifications) return { token: null, reason: 'notifications_module_unavailable' };
...
async function ensurePushPermissionAndTokenWithOptions(options = {}) {
  ...
  try {
    ...
    const Notifications = await loadNotificationsModule();
    if (!Notifications) {
      return { granted: false, token: null, reason: 'notifications_module_unavailable' };
    }
...
export async function syncChangedPushTokenForUser(userId, devicePushToken) {
  ...
  try {
    const Notifications = await loadNotificationsModule();
    if (!Notifications) {
      return { ok: false, skipped: true, reason: 'notifications_module_unavailable' };
    }
    await ensureAndroidNotificationChannel(Notifications);
