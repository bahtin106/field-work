...
export function PermissionsProvider({ children }) {
    ...
    const doRefresh = async () => {
      try {
        ...
        if (!prof) {
          ...
          setRoleIfChanged(null);
          setCompanyIdIfChanged(null);
        ...
        setRoleIfChanged(prof.role || null);
        setCompanyIdIfChanged(prof.company_id || null);
        setFullNameIfChanged(prof.full_name || '');
  ...
  useEffect(() => {
    ...
    setRoleIfChanged(profile.role || null);
    setCompanyIdIfChanged(profile.company_id || null);
    ...
    }, PERMISSIONS_BOOTSTRAP_REFRESH_DELAY_MS);
    return () => {
  ...
  const rolePerms = useMemo(() => {
    if (!role) {
      ...
      return matrix?.worker || START_PRESET.worker || {};
    }
    return matrix?.[role] || {};
  }, [matrix, role]);

  const has = useCallback((permKey) => {
    // Просмотр фото доступен всем без ограничений
    if (
      permKey === 'canViewClients' ||
      permKey === 'canViewObjects' ||
      permKey === 'canCreateClients' ||
      permKey === 'canCreateObjects'
    ) return true;
  ...
  const getMatrix = useCallback(() => matrix, [matrix]);
  ...
  const value = useMemo(
    ...
    [companyId, fullName, getMatrix, has, hasAll, hasAny, loading, matrix, refresh, role, rolePerms, source],
  );
