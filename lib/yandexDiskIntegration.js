...
export async function yandexDiskIntegration(action, payload = {}) {
  return invokeYandexFunction('yandex-disk-integration', { action, ...payload }, getIntegrationFallback());
}

export async function yandexDiskMedia(action, payload = {}) {
  return invokeYandexFunction('yandex-disk-media', { action, ...payload }, getMediaFallback());
}
