// src/i18n/en.js
export default {
  // Page titles (used in AppHeader/getRouteTitle)
  routes: {
    settings: 'Company Settings',
  },
  // Login screen
  login_no_account: 'No account?',

  // Register screen (local overrides)
  register_label_password: 'Password *',
  register_placeholder_password: 'Enter password',

  calendar_toggle: 'Toggle calendar size',
  order_section_address: 'Address',
  order_section_customer: 'Customer',
  order_section_finances: 'Finances',
  order_section_work_type: 'Work type',
  order_field_work_type: 'Work type',
  order_field_departure_date: 'Departure date',
  order_field_departure_time: 'Departure time',
  order_placeholder_departure_date: 'Select date',
  order_placeholder_departure_time: 'Select date first',
  order_validation_work_type_required: 'Please select a work type',
  order_validation_amount_format: 'Enter a valid amount',
  order_validation_fuel_format: 'Enter a valid fuel amount',
  order_load_error: 'Failed to load order',
  order_not_found: 'Order not found',
  order_details_amount: 'Amount',
  order_details_fuel: 'Fuel',
  order_placeholder_amount: '0.00',
  order_status_new: 'New',
  order_status_in_progress: 'In progress',
  order_status_completed: 'Completed',
  common_search: 'Search',
  common_filter: 'Filters',
  common_back: 'Back',
  common_select: 'Select',
  common_from: 'From',
  common_to: 'To',
  common_bullet: ' • ',
  common_required_suffix: ' *',
  fields_phone: 'Phone',
  fields_phone_placeholder: '+7 (___) ___-__-__',
  orders_filter_status: 'Status',
  orders_total: 'Total orders',
  orders_found: 'Found',
  settings_sections_quiet_items_quiet_reset: 'Reset',
  btn_retry: 'Retry',
  err_required_field: 'Required field',
  register_error_consent_required: 'Consent to the privacy policy is required to register',
  btn_apply: 'Apply',
  btn_reset_password: 'Reset password',
  header_save: 'Save',
  header_edit_user: 'Edit',
  dlg_reset_password_title: 'Reset password',
  dlg_reset_password_msg: 'Send a new password to the employee?',
  toast_reset_password_sending: 'Resetting password…',
  toast_reset_password_sent: 'New password sent',
  err_reset_password_failed: 'Failed to reset password',
  user_block_keepOrders: 'Keep orders with employee',
  user_block_keepOrders_desc: 'The employee will be suspended. All their orders will remain with them.',
  user_block_noOrders_desc: 'This employee has no active orders. After suspension, they will not be able to use the app.',
  user_block_reassign: 'Reassign all orders',
  // Finances
  company_settings_sections_finances_title: 'Finances',
  company_settings_sections_finances_items_currency: 'Currency',
  settings_company_currency_label: 'Company currency',
  settings_finances_currency: 'Currency used',
  finance_currency_RUB: 'Russian Ruble',
  finance_currency_USD: 'US Dollar',
  finance_currency_EUR: 'Euro',
  finance_no_currency: 'Not set',
  modal_currency_title: 'Select currency',
  modal_currency_confirm_recalc: 'Recalculate existing orders when changing currency?',
  modal_currency_yes_recalc: 'Recalculate and lock editing',
  modal_currency_no_recalc: 'Change currency without recalculation',
  modal_currency_rate_label: 'Exchange rate (new/old)',
  modal_currency_rate_placeholder: 'e.g. 0.013 (1 RUB → USD)',
  settings_recalc_in_progress: 'Recalculation in progress…',
  errors_only_admin_currency: 'Only company admin can change currency',
  toast_loading_info: 'Loading information…',
  toast_deleting_employee: 'Deleting employee…',
  user_delete_no_orders_title: 'Employee will be removed',
  user_delete_no_orders_desc:
    'This employee has no orders. After deletion, all data and documents will be permanently removed.',
  user_delete_reassign_title: 'Reassign orders',
  user_delete_reassign_desc:
    'This employee has {n} active order(s). Select an employee to reassign all orders before deletion.',
  user_delete_reassigned_title: 'Reassigned orders',
  user_delete_reassigned_desc: '{n} order(s) will be reassigned to the following employee:',
  err_check_orders_failed: 'Failed to check employee orders',
  err_successor_required_delete: 'Select an employee to reassign orders',
  err_delete_failed_status: 'Delete failed ({status})',
  err_delete_failed: 'Delete failed',
  err_deactivate_failed: 'Failed to deactivate employee',
};
