...
function notify() {
  _version++;
  _listeners.forEach(fn => { try { fn(_version); } catch {} });
}

export function getLocale()   { return _locale; }
export function getDict()     { return _dict; }
...
export async function setLocale(next) {
  try {
    ...
    _dict = dict;
    try { await AsyncStorage.setItem(STORAGE_KEY, _locale); } catch {}
    notify();
    return true;
  } catch (e) {
...
export function getLabel(key, fallback) { return t(key, fallback); }
